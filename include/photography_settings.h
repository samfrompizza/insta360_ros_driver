...
struct EnumClassHash {
    template <typename T> std::size_t operator()(T const& v) const noexcept { return static_cast<std::size_t>(v); }
};

enum SubPhotoMode {
    PHOTO_SINGLE = 0,   // Normal photo
    PHOTO_HDR = 1,      // HDR photo
    PHOTO_INTERVAL = 2, // Interval shooting
    PHOTO_BURST = 3,    // Burst photo
    PHOTO_AEB_NIGHT = 4,
    PHOTO_INSTA_PANO = 5,
    PHOTO_INSTA_PANO_HDR = 6,
    PHOTO_STARLAPSE = 7, // Starlapse photo
    PHOTO_NONE = 100
};
...
enum SubVideoMode {
    ...
    VIDEO_TIMESHIFT = 4,     // Moving timelapse (TimeShift)
    VIDEO_SUPER = 5,         // Free aspect ratio
    VIDEO_LOOPRECORDING = 6, // Loop recording
    VIDEO_SLOW_MOTION = 9,   //
    VIDEO_SELFIE = 10,       // Selfie/follow mode
    VIDEO_PURE = 11,         // Night video (PureVideo)
    VIDEO_LIVEVIEW = 12,     // Live streaming X4/X5
    VIDEO_DASH_CAM = 14,     // Dashcam mode
...
enum CameraFunctionMode {
    ...
    FUNCTION_MODE_BULLETTIME = 23, // Bullet time
...
// 3A image-quality mode. PRO unlocks parameters that are disabled in NORMAL (e.g. sharpness).
enum Iq3AMode {
    IQ_3A_INVALID = 0, // invalid / placeholder
    IQ_3A_NORMAL = 1,  // normal mode
    IQ_3A_PRO = 2,     // professional mode
};

enum CameraTimelapseMode {
    TIMELAPSE_MIXED = 0,              // Mixed VIDEO and IMAGE settings. [kept for compatibility]
    MOBILE_TIMELAPSE_VIDEO = 1,       // Settings for moving timelapse
    TIMELAPSE_INTERVAL_SHOOTING = 2,  // Settings for interval shooting
    STATIC_TIMELAPSE_VIDEO = 3,       // Settings for static/fixed-position timelapse
    TIMELAPSE_INTERVAL_VIDEO = 4,     // Settings for interval video
    TIMELAPSE_STARLAPSE_SHOOTING = 5, // Settings for starlapse
};
enum VideoResolution {
    ...
    // X4
    RES_5120_5120P30 = 249,  // 12KP30 timelapse
    ...
    // X4
    RES_1530_2720P120 = 261, // Selfie/follow mode 2.7KP120 (9:16)
    RES_3040_760P240 = 262,  // Bullet time 3KP240
    ...
    RES_5760_1440P120 = 307, // Bullet time 5.7K120P
    ...
    // X5~
    RES_3008_3008P24 = 349, // 6K24
    RES_3008_3008P25 = 350, // 6K25
    RES_3008_3008P30 = 351, // 6K30
    RES_3840_960P240 = 352, // Bullet time 4K240P
    ...
    RES_3008_3008P48 = 367, // X6 6K48
    RES_3008_3008P50 = 368, // X6 6K50
    RES_3008_3008P60 = 369, // X6 6K60
    RES_3840_3840P50 = 402, // X6 8K50
    // X5
    RES_3840_960P200 = 358, // Bullet time 4K200P

    RES_3840_3840P48 = 370, // C9 pano 8K

    RES_8KP48 = RES_3840_3840P48,
    RES_8KP30 = RES_3840_3840P30,
    RES_8KP25 = RES_3840_3840P25,
    RES_8KP24 = RES_3840_3840P24,
    RES_6KP30 = RES_3008_3008P30,
    RES_6KP25 = RES_3008_3008P25,
    RES_6KP24 = RES_3008_3008P24,
    ...
    // X6 supported video resolutions and their frame rates
    // 4K 100 60 50 48 30 25 24
    // 6K 60 50 48 30 25 24
    // 8K 50 48 30 25 24
    RES_8KP50 = RES_3840_3840P50,
    RES_6KP60 = RES_3008_3008P60,
    RES_6KP50 = RES_3008_3008P50,
    RES_6KP48 = RES_3008_3008P48,
...
enum PhotoSize {
    ...
    Size_X4Air_29MP = Size_7680_3840,
    Size_72M = Size_11968_5984,
    Size_X6_120MP = Size_15520_7760,
    Size_X6_30MP = Size_7776_3888,
};
...
public:
    ...
    enum SettingsType {
        ...
        CaptureSettings_Iq3AMode,
        CaptureSettings_RecordResolution,
        CaptureSettings_PhotoResolution
    };
...
struct RecordParams {
    ...
    int32_t bitrate{0};
    int accelerate_fequency{0}; // Speed multiplier, applies to timeshift
    // Loop-recording (VIDEO_LOOPRECORDING) segment length in seconds: when a segment is full,
    // the oldest one is overwritten (rolling window); recording itself does NOT stop.
    // 0 = keep camera default. Query valid values via GetSupportedAttrValues(mode, "record_duration").
    // NOTE: photo timelapse modes (starlapse / interval) also expose an attr named "record_duration",
    // but there it means the TOTAL capture duration (auto-stop) and is set via
    // SetTimeLapseOption(TimelapseParam.duration) — a different channel. Do not confuse the two.
    uint32_t record_duration{0};
};
...
struct TimelapseParam {
    ...
    uint32_t duration;            // Total capture duration, in seconds
    uint32_t lapseTime;           // Interval, in milliseconds
    uint32_t accelerate_fequency; // Timelapse video speed multiplier (moving timelapse)
};
} // namespace ins_camera
